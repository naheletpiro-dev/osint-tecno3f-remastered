...
export default function App() {
  ...
  useEffect(() => {
    if (!loading) {
      setScanProgress(0);
      setScanStageText('Iniciando rastreo OSINT...');
    }
  }, [loading]);
  ...
  // Perform OSINT Scan via Unified API Server
  const handleScan = async ({ companyName, website, region, cuit }) => {
    setLoading(true);
    setError(null);
    setReport(null);
    setComparisonReport(null);
    ...
    eventSource.addEventListener('complete', (e) => {
      try {
        ...
        setReport(data.report);
        setActiveTab('overview');
        if (user) saveReportToHistory(data.report);
      } catch (err) {
  ...
  const handleCompare = async ({ companyA, websiteA, companyB, websiteB }) => {
    ...
    try {
      ...
      if (!response.ok || !data.reportA || !data.reportB) {
        throw new Error(data.error || 'No se pudo completar la comparación entre las empresas.');
      }
      ...
      setError(err.message || 'Error al conectar con el servidor API OSINT para la comparación.');
    } finally {
      setLoading(false);
    }
  };
          ...
          <>
            ...
            {!loading && report && !comparisonReport && !showHistoryOnly && (
              <div>
                ...
                {activeTab === 'news' && <NewsTab searchData={report.searchData || {}} legalData={report.legalData || {}} companyName={companyName} />}
