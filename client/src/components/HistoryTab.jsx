          ...
          {history.map((item, idx) => {
            ...
            return (
              <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '18px 20px', borderRadius: '12px', background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border-subtle)', flexWrap: 'wrap', gap: '14px' }}>
                <div>
                  <div style={{ fontSize: '1.12rem', fontWeight: 700, color: 'var(--text-primary)' }}>{compName}</div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                  <div style={{ textAlign: 'right' }}>
                    ...
                    <div style={{ fontWeight: 800, color: creditScore >= 70 ? 'var(--accent-emerald)' : 'var(--accent-amber)', fontSize: '1.05rem' }}>{creditScore} / 100 ({riskLevel})</div>
                  </div>
