...
export default function LegalTab({ legalData = {}, companyName = '' }) {
  ...
  return (
    <div className="dashboard-grid">
      ...
      {/* Boletín Oficial de la República Argentina (BORA) Card */}
        ...
        <div className="saas-card col-12" style={{ padding: '24px', background: 'linear-gradient(135deg, rgba(37, 99, 235, 0.08), rgba(16, 185, 129, 0.05))', border: '1px solid rgba(37, 99, 235, 0.35)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '14px', marginBottom: '16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                  <span style={{ fontSize: '0.78rem', color: '#60a5fa', textTransform: 'uppercase', fontWeight: 800 }}>Boletín Oficial de la República Argentina (BORA)</span>
                  <RealVsEstimatedBadge isRealData={data.boletinOficialData.isRealData} sourceLabel="timeline.boletinoficial.gob.ar" />
                </div>
                <h3 style={{ fontSize: '1.25rem', fontWeight: 800, marginTop: '4px', color: 'var(--text-primary)' }}>
                  Publicaciones y Edictos Societarios
                </h3>
              </div>
            </div>
            <a
          ...
          <div style={{
            ...
            border: `1px solid ${data.boletinOficialData.hasBankruptcyOrConcurso ? 'var(--accent-rose)' : 'var(--accent-emerald)'}`,
            color: data.boletinOficialData.hasBankruptcyOrConcurso ? '#fca5a5' : 'var(--accent-emerald)',
            padding: '10px 14px',
            borderRadius: '8px',
            fontSize: '0.86rem',
            fontWeight: 800,
              ...
              {data.boletinOficialData.edicts.map((ed, idx) => (
                <div key={idx} style={{ background: 'rgba(0,0,0,0.3)', padding: '12px 16px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.05)', fontSize: '0.85rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                    <strong style={{ color: 'var(--text-primary)' }}>{ed.title}</strong>
      ...
      {data.repsalData && (
        <div className="saas-card col-12" style={{ padding: '24px', background: data.repsalData.hasSanctions ? 'linear-gradient(135deg, rgba(239, 68, 68, 0.08), rgba(245, 158, 11, 0.05))' : 'linear-gradient(135deg, rgba(16, 185, 129, 0.06), rgba(37, 99, 235, 0.05))', border: data.repsalData.hasSanctions ? '1px solid rgba(239, 68, 68, 0.35)' : '1px solid rgba(16, 185, 129, 0.3)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px', flexWrap: 'wrap', gap: '10px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
              <Scale size={20} style={{ color: data.repsalData.hasSanctions ? 'var(--accent-rose)' : 'var(--accent-emerald)' }} />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {data.repsalData.sanctions.map((s, idx) => (
                <div key={idx} style={{ background: 'rgba(0,0,0,0.3)', padding: '12px 16px', borderRadius: '10px', border: '1px solid rgba(239,68,68,0.2)', fontSize: '0.85rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                    <strong style={{ color: 'var(--text-primary)' }}>{s.sanctionType}</strong>
                    <span style={{ fontSize: '0.74rem', background: 'rgba(239,68,68,0.2)', color: 'var(--accent-rose)', padding: '2px 8px', borderRadius: '6px', fontWeight: 800 }}>{s.status}</span>
                  </div>
