  ...
  const handleSendMessage = async (textToSend) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const botMsg = { id: Date.now() + 1, sender: 'bot', text: data.answer || 'No se pudo obtener respuesta.' };
      ...
      {isOpen && (
        <div
          style={{
            ...
            width: window.innerWidth <= 480 ? '100%' : '390px',
            maxHeight: window.innerWidth <= 480 ? '100vh' : '600px',
            height: window.innerWidth <= 480 ? '100vh' : '80vh',
        ...
        >
          {/* Chat Header */}
          <div style={{ padding: '16px 20px', background: 'linear-gradient(135deg, rgba(139, 92, 246, 0.2), rgba(6, 182, 212, 0.15))', borderBottom: '1px solid rgba(139, 92, 246, 0.3)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ flex: 1, padding: '16px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            {loading && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#c4b5fd', fontSize: '0.82rem', padding: '6px 12px' }}>
                <RefreshCw size={14} className="spinner" /> Asistente OSINT analizando datos...
              </div>
            )}
