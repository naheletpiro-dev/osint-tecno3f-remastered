...
export default function WatchlistModal({ isOpen, onClose, currentCompany = null }) {
  ...
  const getAuthHeaders = () => {
    let token = localStorage.getItem('osint_auth_token');
    if (!token) {
      try {
        const userStr = sessionStorage.getItem('osint_user') || localStorage.getItem('osint_user');
        if (userStr) {
  ...
  return (
    <div style={{
      ...
      <div className="saas-card" style={{ width: '100%', maxWidth: '780px', maxHeight: '90vh', overflowY: 'auto', padding: '32px', border: '1px solid rgba(56, 189, 248, 0.3)', boxShadow: '0 20px 60px rgba(0,0,0,0.6)' }}>
        ...
        <form onSubmit={handleAddCompany} style={{ background: 'rgba(255, 255, 255, 0.02)', border: '1px solid var(--border-subtle)', padding: '18px', borderRadius: '12px', marginBottom: '24px', display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'flex-end' }}>
          <div style={{ flex: '1 1 200px' }}>
            ...
            <input
              ...
              style={{ width: '100%', padding: '10px 14px', background: 'rgba(0,0,0,0.4)', border: '1px solid var(--border-subtle)', borderRadius: '8px', color: 'var(--text-primary)', fontSize: '0.9rem', marginTop: '4px' }}
            />
          </div>
        ...
        {successMsg && <div style={{ background: 'rgba(16,185,129,0.12)', border: '1px solid rgba(16,185,129,0.3)', color: 'var(--accent-emerald)', padding: '10px 14px', borderRadius: '8px', marginBottom: '16px', fontSize: '0.85rem' }}>{successMsg}</div>}
            ...
            {watchlist.map(item => (
              <div key={item.id} style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid var(--border-subtle)', padding: '16px', borderRadius: '12px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px', marginBottom: '10px' }}>
                  <div>
                    <h4 style={{ fontSize: '1.1rem', fontWeight: 800, color: 'var(--text-primary)' }}>{item.companyName}</h4>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <span style={{ background: 'rgba(16, 185, 129, 0.12)', color: 'var(--accent-emerald)', border: '1px solid rgba(16, 185, 129, 0.3)', padding: '4px 10px', borderRadius: '12px', fontSize: '0.74rem', fontWeight: 700 }}>
                      ✔ {item.status}
                    </span>
                ...
                {item.alerts && item.alerts.length > 0 && (
                  <div style={{ marginTop: '10px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    {item.alerts.slice(0, 3).map((alt, idx) => (
                      <div key={idx} style={{ background: alt.severity === 'warning' ? 'rgba(245, 158, 11, 0.08)' : 'rgba(255,255,255,0.02)', borderLeft: `3px solid ${alt.severity === 'warning' ? 'var(--accent-amber)' : '#38bdf8'}`, padding: '8px 12px', borderRadius: '6px', fontSize: '0.82rem' }}>
                        <div style={{ fontWeight: 700, color: alt.severity === 'warning' ? 'var(--accent-amber)' : '#60a5fa' }}>{alt.title}</div>
