...
app.get('/api/osint/scan-stream', scanRateLimiter, async (req, res) => {
  ...
  const cleanCuit = (cuit && isValidCuit(cuit)) ? String(cuit).replace(/\D/g, '') : null;
  ...
  try {
    ...
    let [inpiWipoData, openCorporatesData, publicContracts, tradeData, pymeData, legalData, categorization, digitalTransformation] = await Promise.all([
