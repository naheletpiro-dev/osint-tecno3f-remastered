...
export async function analyzeLegalOSINT(companyName, domainAnalysis = {}, cuit = '') {
  ...
  if (sociedadesDb) {
    try {
      ...
      if (!rec && cleanComp) {
        // Enforce exact match instead of LIKE %...% to avoid non-deterministic data collisions
        rec = sociedadesDb.get('SELECT * FROM sociedades_registry WHERE razon_social COLLATE NOCASE = ? LIMIT 1', [cleanComp]);
      }
