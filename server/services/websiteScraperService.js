...
export function deduplicateItems(items, companyName = '') {
  ...
  const GENERIC_NOISE_TERMS = [
    ...
    'ver mas', 'ver más', 'home', 'inicio', 'contacto', 'nosotros', 'acerca de', 'mas informacion',
    'más información', 'politica de privacidad', 'terminos y condiciones', 'todos los derechos',
    'soluciones', 'soluciones integrales', 'dos tipos de', 'una misma plataforma', 'bienvenidos'
  ];
  ...
  sorted.forEach(rawItem => {
    ...
    if (item.length < 5 || item.length > 95) return;

    const norm = item.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').trim();

    // Filter exact match or starting match with generic noise headers
    const isNoise = GENERIC_NOISE_TERMS.some(noise => norm === noise || norm.startsWith(noise + ':') || norm === noise + 's');
    if (isNoise) return;

    // Reject generic marketing slogans that aren't concrete product/service names
    if (norm.includes('dos tipos de') || norm.includes('una misma') || norm.includes('haga clic') || norm.includes('todos los derechos')) {
      return;
    }
  ...
  return cleanedItems.reverse().slice(0, 10);
}
...
async function discoverSitemapUrls(baseUrl, targetHost) {
  ...
  try {
    ...
    const robotsRes = await safeAxiosGet(robotsUrl, { timeout: 1500 }).catch(() => null);
  ...
  for (const smUrl of sitemapCandidates) {
    try {
      const res = await safeAxiosGet(smUrl, {
      ...
      if (res && res.data && typeof res.data === 'string' && (res.data.includes('<urlset') || res.data.includes('<sitemapindex') || res.data.includes('<loc>'))) {
        ...
        if (childSitemaps.length > 0) {
          for (const cSm of childSitemaps) {
            try {
              const cRes = await safeAxiosGet(cSm, { timeout: 1500 }).catch(() => null);
              if (cRes && cRes.data) {
...
/**
 * Fallback Scraper for SPAs (React, Angular, Vue, Next.js) using Puppeteer.
 * Opens a headless browser, waits for network idle, and returns the rendered HTML.
 */
  ...
  try {
    browser = await puppeteer.launch({
      ...
      args: [
        ...
        '--disable-dev-shm-usage', // crucial for Docker/Render (low RAM)
    ...
    console.warn(`[PUPPETEER ERROR] Fallback failed for ${url}:`, error.message);
    return null;
  } finally {
...
export async function scrapeCompanyWebsite(websiteUrl, companyName) {
  ...
  try {
    const response = await safeAxiosGet(formattedUrl, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
        'Accept-Language': 'es-ES,es;q=0.9,en;q=0.8'
      },
    ...
    if (response.data) {
      ...
      let unstrippedFullText = $('body').text().replace(/\s+/g, ' ').trim();

      // SPA Detection & Dynamic Headless Fallback
      if (unstrippedFullText.length < 150 || htmlContent.includes('id="root"') || htmlContent.includes('id="__next"')) {
        console.log(`[ADVANCED CRAWLER] Client-Side Rendered (SPA) detected on ${formattedUrl}. Executing Puppeteer fallback...`);
        const puppeteerHtml = await scrapeWithPuppeteer(formattedUrl);
        if (puppeteerHtml) {
          htmlContent = puppeteerHtml;
          $ = cheerio.load(htmlContent);
          profile.title = $('title').text().trim() || profile.title;
          profile.description = $('meta[name="description"]').attr('content') || profile.description;
          unstrippedFullText = $('body').text().replace(/\s+/g, ' ').trim();
        }
      }
...
function mergeUniversalFallbackData(companyName, profile) {
  ...
  if (dynamicClients.length === 0) {
    if (combinedLower.includes('smartmation') || combinedLower.includes('telegestión') || combinedLower.includes('iot')) {
      ...
      dynamicClients = [
        'Por el momento el dato no está disponible.'
      ];
    }
  }
  ...
  const whatItSells = `${cleanComp} comercializa de forma verificada: ${dynamicProducts.join(', ')}.`;
